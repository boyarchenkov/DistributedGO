...
namespace M.Tools
{
	public class Utility
	{
        ...
        public static List<T> Parse<T>(ICollection<string> c, ParseDelegate<T> parser)
        {
            List<T> list = new List<T>();
            foreach (string item in c) list.Add(parser(item));
            return list;
        }
        ...
        public static Float MaxAbs(params Float[] x)
        {
            Float y = Math.Abs(x[0]);
            for (int i = 1; i < x.Length; i++)
            {
                Float xi = Math.Abs(x[i]);
                if (xi > y) y = xi;
            }
            return y;
        }
